...
#' @importFrom ghelper make.bins.gr
setupDomainModel <- function(isot, mod.dir, data.types = isot@data.types){
      
      domain <- isot@domain
      bin.width <- isot@bin.width
      chrlen.file <- isot@chrlen.file
...
generateModelFile <- function(isot, id = 1:length(isot@domain)){
      ...
      for (j in 1:length(id)){
            ...
            for (dtype in data.types) {
                  ...
                  # avoid loading the raw matrix multiple times
                  if (chr != chr.last){
