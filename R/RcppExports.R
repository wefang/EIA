# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

SearchMode <- function(mat, bg_mean, bg_sd, K, max_iter, tol, p, q, theta1, sigma1) {
    .Call('EIA_SearchMode', PACKAGE = 'EIA', mat, bg_mean, bg_sd, K, max_iter, tol, p, q, theta1, sigma1)
}

calLikeIso <- function(mat, p, q, bg_mean, bg_sd, theta1, sigma1) {
    .Call('EIA_calLikeIso', PACKAGE = 'EIA', mat, p, q, bg_mean, bg_sd, theta1, sigma1)
}

calPostProb <- function(mat, p, q, bg_mean, bg_sd, theta1, sigma1) {
    .Call('EIA_calPostProb', PACKAGE = 'EIA', mat, p, q, bg_mean, bg_sd, theta1, sigma1)
}

run_em <- function(mat, bg_mean, bg_sd, K, max_iter, tol, num_threads, p, q, theta1, sigma1, theta1_0, sigma1_0, nu_0 = 2.0) {
    .Call('EIA_run_em', PACKAGE = 'EIA', mat, bg_mean, bg_sd, K, max_iter, tol, num_threads, p, q, theta1, sigma1, theta1_0, sigma1_0, nu_0)
}

