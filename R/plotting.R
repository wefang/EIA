...
plotIsoformModel <- function(isom, zlim = c(0, 8), col.sel = NULL, main = "", cell.names = ""){
    ...
    layout(matrix(c(3, 4, 1, 2), 2), heights=c(5, 1.5), widths=c(3, 10))

    # original matrix
    par(mar = c(0, 0, 2, 0.8))
    gheat(t(isom@mat[ord, col.sel]), zlim, col = brewer.pal(9, "YlGn"),
             axes=FALSE, rowsep = isoform.sep, sepcolor = "#2171b5", sepwidth = 0.05)
    title(main, line = 0.5, cex = 1)

    # isoform matrix
    par(mar = c(1, 0, 1, 0.8))
    gheat(t(q.mat[q.ord, col.sel, drop=F]), col = brewer.pal(9, "Blues"),
             zlim = c(0, 1),rowsep = 1:(length(isoform.index) - 1), axes=FALSE)

    # isoform indicator
    par(mar = c(0, 5, 2, 0))
    gheat(t(isoform.dum), zlim = c(1, max(isoform.dum, na.rm = T)),
    ...
    par(mar = c(1, 5, 1, 0))
    gheat(t(matrix(isom@p[isoform.index[q.ord]])), zlim = c(0, 1), col = brewer.pal(9, "PuRd"), cellnote = T, axes = F,
